...
import { AGE_RANGES, type AgeRangeId } from "@/lib/types/onboarding"

interface AgeStepProps {
  selected: AgeRangeId | null
  onSelect: (id: AgeRangeId) => void
  onNext: () => void
  onBack: () => void
}
    ...
    <div className="space-y-5">
      <div className="flex items-start gap-3 rounded-2xl border border-[#1A8A7A]/15 bg-[#1A8A7A]/5 p-3.5">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white">
          <Calendar className="h-4 w-4 text-[#1A8A7A]" />
        </div>
        <div className="text-xs leading-relaxed text-[#2D3748]/70">
          <strong className="font-bold text-[#0D1B2A]">Kenapa nanya usia?</strong>{" "}
          Biar kami bisa kasih saran &amp; cara ngobrol yang pas buat kamu.
        </div>
      </div>
