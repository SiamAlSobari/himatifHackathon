...
- **Daftar Psikolog:** foto, spesialisasi, rating, harga, jadwal.
- **Booking Flow:**
  1. Pilih psikolog → pilih slot waktu.
  2. Pembayaran (sesi 30/60 menit, paket).
...
### Alur Monetisasi
1. **Gratis:** Screening, AI Chatbot, profil, lihat daftar psikolog.
2. **Berbayar:** Sesi konsultasi dengan psikolog.
3. **Pricing per Sesi:**
   - 30 menit = Rp 150.000
   - 60 menit = Rp 250.000
4. **Paket Bundling** (post-MVP): 4 sesi/bulan diskon 15%.

### Booking & Approval
1. Pengguna pilih slot → bayar via payment gateway (Midtrans/Xendit/Stub).
2. Status: `pending_payment` → `pending_psychologist` → `confirmed` / `declined` (auto-refund).
3. Psikolog punya **24 jam** untuk approve/decline.
4. Saat jadwal, chat dibuka (real-time websocket).
...
| `appointments` | Booking, status, payment |
| `consultation_notes` | Catatan pasca-sesi psikolog |
| `payments` | Transaksi payment gateway |
...
| **Payment gagal** | Revenue hilang | Auto-refund + retry. Mock payment untuk demo. |
...
4. **Payment MVP** — mock/stub untuk demo
