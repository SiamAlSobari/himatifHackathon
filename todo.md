...
> 4. `ChatPanel.tsx` masih pakai **`useState` + mock data** — **belum nyambung** ke `/api/ai/chat` & `/api/ai/session`. Backend-nya sudah jalan.
> 5. `schema.prisma` **belum ada** model `Psychologist`, `SessionSummary`, `Appointment`, `ConsultationNote`, `Payment`. Harus ditambah sebelum FASE 4.
...
- [~] Halaman chatbot: sidebar sesi + chat area
  ...
  - [ ] `ChatPanel` **masih mock**: `useState` + 3 pesan hardcode, `handleSend` cuma append ke array — **TIDAK panggil API**
...
- [ ] Halaman list psikolog (filter spesialisasi, harga) — **belum**
...
- [ ] Tambah `Psychologist` model: id, name, email, spesialisasi, bio, foto, hargaPer30, hargaPer60, rating, verified
- [ ] Tambah `SessionSummary` model: id, chatSessionId, content, uiTheme, createdAt
- [ ] Tambah `Appointment` model: id, userId, psychologistId, scheduledAt, durationMin, status (pending_payment/pending_psychologist/confirmed/declined/completed), paymentId?
- [ ] Tambah `ConsultationNote` model: id, appointmentId, psychologistId, content, createdAt
- [ ] Tambah `Payment` model: id, appointmentId, amount, status, method, gatewayRef, createdAt
- [ ] `npx prisma migrate dev --name add_booking_models`
- [ ] Tulis `prisma/seed.ts` untuk psikolog dummy

### Booking & Payment (Jum, 19/06) ⏳ 0% DONE
- [ ] Halaman booking: pilih slot
- [x] **Keputusan:** payment mock untuk MVP (sudah diputuskan ✅)
- [ ] Status flow: `pending_payment` → `pending_psychologist` → `confirmed` / `declined`
- [ ] Notifikasi ke psikolog (in-app)
- [ ] Halaman payment mock (UI tombol "Bayar" → set status `pending_psychologist`)
...
- [ ] Siapkan script demo (5 menit):
  ...
  3. Booking + payment mock (1 menit)
...
3. **`register.ts` field `usia` & `jenisKelamin` orphan**
   - Server action baca `formData.get("usia")` & `"jenisKelamin")` tapi form sudah tidak punya input itu (lihat progress.md 12/06 17:00 WIB).
   - **Fix:** Hapus dari server action, atau kembalikan ke form (tapi PRD usia/jk di onboarding, bukan register).

4. **`ChatPanel.tsx` tidak konek API**
   - `useState<Message[]>(initialMessages)` hardcode 3 pesan.
   - `handleSend` cuma `setMessages` — tidak panggil `/api/ai/chat`.
   - **Fix:** Pakai `useEffect` untuk create session + load messages, panggil `fetch('/api/ai/chat', { method: 'POST' })` di `handleSend`.
